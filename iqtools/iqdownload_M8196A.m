...
%    opts = '001';
    ...
    if (fs ~= 0)
        ...
        xfprintf(f, sprintf('%s:FREQuency:RASTer %.15g;', cmd, fs));
    end
    ...
    for ch = find(channelMapping(:,1))'
        gen_arb_M8196A(arbConfig, f, ch, real(data), marker1, segmNum, run, fs);
    end
    for ch = find(channelMapping(:,2))'
        gen_arb_M8196A(arbConfig, f, ch, imag(data), marker2, segmNum, run, fs);
    end
...
function gen_arb_M8196A(arbConfig, f, chan, data, marker, segm_num, run, fs)
% download an arbitrary waveform signal to a given channel and segment
    if (isempty(chan) || ~chan)
        return;
    end
    if (isfield(arbConfig, 'peaking'))
        xfprintf(f, sprintf(':OUTP%d:VPCORR %f', chan, arbConfig.peaking));
    end
