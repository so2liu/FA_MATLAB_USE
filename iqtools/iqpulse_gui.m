...
% Last Modified by GUIDE v2.5 13-Jun-2015 04:38:45
...
switch arbConfig.model
    ...
    case 'M933xA'
        offset = 0;
        span = 500e6;
    case { 'M8195A_Rev0' 'M8195A_Rev1' 'M8195A_1ch' 'M8195A_2ch_256k' 'M8195A_4ch_256k' 'M8196A' }
        offset = 10e9;
        span = 1e9;
        pw = 1.6e-6;
        pri = 4e-6;
...
set(handles.editAmplitude, 'String', '0');
set(handles.popupmenuShape, 'Value', 1);
set(handles.popupmenuChirp, 'Value', 9);
popupmenuChirp_Callback(handles.popupmenuChirp, eventdata, handles);
...
if (~isempty(vsaApp))
    ...
    vsafunc(vsaApp, 'freq', arbConfig.defaultFc + fc, span, 51201, 'uniform', 0, tlen);
    vsafunc(vsaApp, 'trace', 4, 'Chirp');
    vsafunc(vsaApp, 'start', 1);
...
function [iqdata, marker, sampleRate] = calculate_pulse(handles, doCode)
% handles    structure with handles and user data (see GUIDATA)
...
if (doCode)
    ...
    segmentNum = evalin('base', get(handles.editSegment, 'String'));
    fsStr = sprintf('fs = %g;\n', sampleRate);
    iqdata = [sprintf([fsStr '[iqdata, marker] = iqpulse(''sampleRate'', fs, ...\n' ...
    '    ''PRI'', %s, ''PW'', %s, ...\n' ...
    '    ''riseTime'', %s, ''fallTime'', %s, ...\n' ...
    '    ''pulseShape'', ''%s'', ''span'', %s, ''offset'', %s, ...\n' ...
    '    ''amplitude'', %s, ''fmFormula'', ''%s'', ''pmFormula'', ''%s'', ...\n' ...
    '    ''modulationType'', ''%s'', ''correction'', %d, ''delay'', %s, ''phase'', %s);\n' ...
    'iqdownload(iqdata, fs, ''channelMapping'', %s, ...\n' ...
    '    ''segmentNumber'', %d, ''marker'', marker);\n'], ...
        vectorStr(pri), vectorStr(pw), vectorStr(riseTime), vectorStr(fallTime), ...
        shapeList{shapeIdx}, vectorStr(span_f), vectorStr(offset_f), ...
        vectorStr(amplitude), FMFormula, PMFormula, modulationList{modulationIdx}, ...
        correctFlag, vectorStr(delay), vectorStr(phase), channelMapping, segmentNum)];
else
    ...
    [iqdata, marker] = iqpulse(...
        ...
        'correction', correctFlag, 'delay', delay, 'phase', phase);
    catch ex
       errordlg({ex.message, [ex.stack(1).name ', line ' num2str(ex.stack(1).line)]});
       iqdata = [];
       marker = [];
