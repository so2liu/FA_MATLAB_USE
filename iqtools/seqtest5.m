...
if (setupSequence)
    ...
    for i = [1 3]
        ...
        seq(i).segmentLoops = 2;
        seq(i).sequenceLoops = 3;    % this parameter is irrelevant, since the sequence will be looped until a new dynamic seq. is selected
        seq(i).markerEnable = true;
        seq(i).segmentAdvance = advanceMode;
        seq(i).sequenceInit = 1;
        
        seq(i+1).segmentNumber = i+1;
        seq(i+1).segmentLoops = 1;
        seq(i+1).markerEnable = false;
        seq(i+1).segmentAdvance = advanceMode;
        seq(i+1).sequenceEnd = 1;
    end
...
if (dyn)
    fprintf(f, ':abort');
    fprintf(f, ':func1:mode sts');
    fprintf(f, ':func2:mode sts');
...
if (pingpong)
    ...
    while true
        ...
        iqdownload(complex(real(sig), real(sig)), fs, 'segmentNumber', 2*mod(pingpong,2)+1, 'keepOpen', 1, 'run', -1);
        pause(1);
        if (updateDialog(hMsgBox, sprintf('Generating Segment #%d ...', pingpong))); break; end
        % ...and select the associated sequence table entry
        fprintf(f, sprintf(':stab:dyn:sel %d', 2*mod(pingpong,2)));
        pingpong = pingpong + 1;
    end
end
