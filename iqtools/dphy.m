...
% Generate a C-PHY signal on two M8190A modules or one M8195A module.
...
%	trigLev = 100e-3;
    ...
    case {3 4} % eye diagram
        ...
        sc = 6; % waveform will span <sc> vertical divisions on scope
...
if (~isempty(strfind(awg1.model, 'M8195A')))
    ...
    w2 = [];
    for i=1:size(wfms,1)
        sum = [sum; [real(wfms{i,1})' imag(wfms{i,1})']];
    end
    if (~doDeskew && length(g_skew) == 4 && length(fixedSkew) == 4)
        sum(:,1) = iqdelay(sum(:,1), fs, g_skew(1)+fixedSkew(1));
        sum(:,2) = iqdelay(sum(:,2), fs, g_skew(2)+fixedSkew(2));
        sum(find(sum > 1)) = 1;
        sum(find(sum < -1)) = -1;
    end
    iqdownload(sum(:,1), fs, 'arbConfig', awg1, 'channelMapping', [1 0; 0 0; 0 0; 0 0], 'run', 0);
    iqdownload(sum(:,2), fs, 'arbConfig', awg1, 'channelMapping', [0 0; 1 0; 0 0; 0 0], 'run', 0);
else
    for i=1:size(wfms,1)
...
if (~isempty(strfind(awg1.model, 'M8195A')))
    sum = [];
    for i=1:size(wfms,1)
        sum = [sum; [real(wfms{i,2})' imag(wfms{i,2})']];
    end
    if (~doDeskew && length(g_skew) == 4 && length(fixedSkew) == 4)
        sum(:,1) = iqdelay(sum(:,1), fs, g_skew(3)+fixedSkew(3));
        sum(:,2) = iqdelay(sum(:,2), fs, g_skew(4)+fixedSkew(4));
        sum(find(sum > 1)) = 1;
        sum(find(sum < -1)) = -1;
    end
    iqdownload(sum(:,1), fs, 'arbConfig', awg1, 'channelMapping', [0 0; 0 0; 1 0; 0 0], 'run', 0);
    iqdownload(sum(:,2), fs, 'arbConfig', awg1, 'channelMapping', [0 0; 0 0; 0 0; 1 0], 'run', 1);
elseif (~isempty(awg2))
    for i=1:size(wfms,1)
...
if (strcmp(cmd, 'init'))
    if (fs > 12e9)  % assume M8195A
        t = (1:1920)/1920;
        t1 = sum(sin((1:2:959)' * (2*pi*t)) .* repmat(1./(1:2:959)', 1, length(t)));
    else  % assume M8190A
...
%            hold on;
            t1 = real(iqcorrection(t1, fs, [], 0));
%            plot(t1(1:500), 'r.-');
            t2 = real(iqcorrection(t2, fs, [], 0));
            t3 = real(iqcorrection(t3, fs, [], 0));
            t4 = real(iqcorrection(t4, fs, [], 0));
        end
...
if (useM8195A)
    del12 = doScopeMeasurement(fscope, [1 2], timebase(1), measDelay(1));
    del13 = doScopeMeasurement(fscope, [1 3], timebase(1), measDelay(1));
    del14 = doScopeMeasurement(fscope, [1 4], timebase(1), measDelay(1));
    newTimebase = max(abs([del12 del13 del14 200e-12])) / 4;
    del12 = doScopeMeasurement(fscope, [1 2], newTimebase, measDelay(1));
    del13 = doScopeMeasurement(fscope, [1 3], newTimebase, measDelay(1));
    del14 = doScopeMeasurement(fscope, [1 4], newTimebase, measDelay(1));
