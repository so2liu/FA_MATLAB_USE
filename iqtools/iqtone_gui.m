...
% Last Modified by GUIDE v2.5 14-Nov-2014 02:05:16
...
[tone mag] = calc_tones(handles);

% check consistency
if (freqOffset >= 0 && min(tone + freqOffset) <= 0 && ...
        get(handles.popupmenuCalType, 'Value') == 1)
   errordlg('can''t measure negative frequencies with spectrum analyzer. Please check Frequency Offset value'); 
   return;
end
...
switch (get(handles.popupmenuCalType, 'Value'))
    case 1  % spectrum analyzer (zero span)
        res = iqcal('tone', tone, 'offset', freqOffset, 'recalibrate', recal, 'method', 'zero span', 'msgbox', hMsgBox);
    case 2  % spectrum analyzer (marker)
        res = iqcal('tone', tone, 'offset', freqOffset, 'recalibrate', recal, 'method', 'marker', 'msgbox', hMsgBox);
    case 3  % spectrum analyzer (list sweep)
        res = iqcal('tone', tone, 'offset', freqOffset, 'recalibrate', recal, 'method', 'list sweep', 'msgbox', hMsgBox);
    case 4  % spectrum analyzer (single tone)
        res = iqcal('tone', tone, 'offset', freqOffset, 'recalibrate', recal, 'method', 'single tone', 'samplerate', fs, 'correction', correction, 'msgbox', hMsgBox);
    case 5  % VSA Software
        res = iqvsamtone('tone', tone, 'fc', freqOffset, 'recalibrate', recal, 'useHW', useHW);
    case 6  % Skew/Ampl/Offset
        res = iqvsaskew('tone', tone, 'fc', freqOffset, 'recalibrate', recal);
end
try
    close(hMsgBox);
catch e;
...
if (notchEnable ~= 0)
    ...
    for i=1:length(notchFreq)
        mag(find(tone >= notchFreq(i) - notchSpan(i)/2 & tone <= notchFreq(i) + notchSpan(i)/2)) = notchDepth(i);
    end
end
...
if (numTones == 0)
    ...
    if (exist('doCode', 'var'))
        ...
        segmentNum = evalin('base', get(handles.editSegment, 'String'));
        iqdata = sprintf(['fs = %g;\niqdata = iqnoise(''sampleRate'', fs, ...\n' ...
            '    ''numSamples'', %g, ''start'', %s, ''stop'', %s, ...\n' ...
            '    ''notchFreq'', %s, ''notchSpan'', %s, ''notchDepth'', %s);\n' ...
            'iqdownload(iqdata, fs, ''channelMapping'', %s, ...\n' ...
            '    ''segmentNumber'', %d);\n'], ...
            sampleRate, points, startFreqStr, stopFreqStr, notchFreqStr, ...
            notchSpanStr, notchDepthStr, channelMapping, segmentNum);
    else
        iqdata = iqnoise('sampleRate', sampleRate, 'numSamples', points, ...
    ...
    if (exist('doCode', 'var'))
        ...
        channelMapping = iqchannelsetup('arraystring', get(handles.pushbuttonChannelMapping, 'UserData'));
        segmentNum = evalin('base', get(handles.editSegment, 'String'));
        toneStr = ['tone = ' strtrim(tone) ';\n'];
        magStr = ['magnitude = [' strtrim(sprintf('%g ', mag)) '];\n'];
        iqdata = [sprintf([fsStr toneStr magStr phaseStr 'iqdata = iqtone(''sampleRate'', fs, ''numSamples'', %g, ...\n' ...
        '    ''tone'', tone, ''phase'', ' phase ', ...\n' ...
        '    ''magnitude'', magnitude, ''correction'', %d);\n' ...
        'iqdownload(iqdata, fs, ''channelMapping'', %s, ...\n' ...
        '    ''segmentNumber'', %d);\n'], points, correction, channelMapping, segmentNum)];
    else
        [tone, mag] = calc_tones(handles, 0);
        iqdata = iqtone('tone', tone, 'numSamples', points, ...
...
if (~isfield(saConfig, 'connected') || ~saConfig.connected)
    set(handles.popupmenuCalType, 'Value', 2);
end
% --- generic checks
