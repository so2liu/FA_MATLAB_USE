...
if (iqoptcheck([], 'DUC', 'DUC'))
    ...
    fs = arbConfig.maximumSampleRate;
...
if (iqoptcheck([], 'DUC', 'DUC'))
    arbConfig = loadArbConfig();
    iqdata = iqpulse('sampleRate', arbConfig.maximumSampleRate, ...
        'PRI', 1000e-9, 'PW', 40e-9, 'risetime', 0, 'falltime', 0, 'span', 0);
    iqdownload(iqdata, arbConfig.maximumSampleRate, 'run', 0, 'keepOpen', 1);
    fcFormula = '1e9';
...
clear seq;
segLoop = round(cycleTime * arbConfig.maximumSampleRate / n / len);
i = 1;
seq(i).segmentNumber = 1;
