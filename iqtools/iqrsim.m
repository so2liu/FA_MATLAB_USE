...
if (showMsgBox)
    hMsgBox = msgbox('Simulation in progress. Please wait...', 'Please wait...', 'replace');
...
%    grid(ax(4), 'on');
    ...
    if (exist('hMsgBox', 'var') && ~isempty(hMsgBox))
        try
            figure(hMsgBox);    % make sure the msgbox is on the top
            ch = get(hMsgBox, 'Children');
            set(get(ch(1), 'Children'), 'String', sprintf('Performing simulation... (%d %%)', round(i/numSteps*100)));
        catch ex
            break;
        end
    end
    ...
    if (triggerMode ~= 1)
        ...
        seq(seqIdx).segmentLoops = max(640,floor(pri * fs) - pSamples); % idle duration
