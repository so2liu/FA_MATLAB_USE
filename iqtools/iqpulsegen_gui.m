...
if (strcmp(contents{get(handles.popupmenuPulseShape,'Value')}, 'Gaussian'))
...
[pulse rpt sampleRate] = calcPulse(handles, 0);
pulse = complex(pulse, pulse);
pulse = repmat(pulse, 1, rpt);
assignin('base', 'pulse', pulse);
segmentNum = evalin('base', get(handles.editSegment, 'String'));
iqdownload(pulse, sampleRate, 'channelMapping', channelMapping, ...
...
function [pulse rpt sampleRate] = calcPulse(handles, doCode)
%try
sampleRate = evalin('base',get(handles.editSampleRate, 'String'));
pw = evalin('base', ['[' get(handles.editPWSamples, 'String') ']']);
rise = evalin('base', ['[' get(handles.editRiseSamples, 'String') ']']);
...
if (doCode)
    ...
    pulse = sprintf(['[pulse, rpt] = iqpulsegen(''sampleRate'', %s, ...\n' ...
        '              ''pw'', %s, ''rise'', %s, ''fall'', %s, ...\n' ...
        '              ''off'', %s, ''low'', %s, ''high'', %s, ...\n' ...
        '              ''pulseShape'', ''%s'', ''alpha'', %d, ''correction'', %d);\n' ...
        'pulse = complex(pulse, pulse);\n' ...
        'pulse = repmat(pulse, 1, rpt);\n' ...
        'iqdownload(pulse, %s, ''channelMapping'', %s, ''segmentNumber'', %d);\n'], ...
        ...
        iqengprintf(sampleRate), channelMapping, segmentNum);
else
    sampleRate = evalin('base',get(handles.editSampleRate, 'String'));
    pw = evalin('base', ['[' get(handles.editPWSamples, 'String') ']']);
    rise = evalin('base', ['[' get(handles.editRiseSamples, 'String') ']']);
    ...
    high = evalin('base', ['[' get(handles.editHigh, 'String') ']']);
    corr = get(handles.checkboxCorrection, 'Value');
    [pulse rpt] = iqpulsegen('sampleRate', sampleRate, ...
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
[code rpt sampleRate] = calcPulse(handles, 1);
