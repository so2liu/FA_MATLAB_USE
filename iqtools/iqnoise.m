function iqdata = iqnoise(varargin)
...
%iqdata = wgn(numSamples, 1, 0, 'complex');
%iqf = fftshift(fft(iqdata));
%-- better approach: start with random phases in frequency domain
iqf = exp(j*2*pi*rand(numSamples,1));
% apply filter
cp = length(iqf)/2 + 1;
