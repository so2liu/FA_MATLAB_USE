...
% Last Modified by GUIDE v2.5 29-Jun-2016 15:10:57
...
switch arbConfig.model
    ...
    case 'M8190A_prototype'
        dataRate = 1e9;
        numBits = 200;
    case { 'M8195A_Rev1' 'M8195A_1ch', 'M8195A_1ch_mrk', 'M8195A_2ch', 'M8195A_2ch_mrk', 'M8195A_4ch', 'M8195A_4ch_256k', 'M8196A' }
        dataRate = 6e9;
        numBits = 1024;
    case 'M933xA'
        dataRate = 250e6;
...
if (isvector(value) && min(value) >= 0 && max(value) <= 5)
...
if (strcmp(dataType, 'PAM4'))
    ...
    set(handles.popupmenuDataFormat, 'Value', 2);
end
...
if (checkM8195A)
    arbConfig = loadArbConfig();
    if (~strcmp(arbConfig.model, 'M8195A_1ch') && ~strcmp(arbConfig.model, 'M8195A_2ch') && ~strcmp(arbConfig.model, 'M8195A_2ch_mrk') && ~strcmp(arbConfig.model, 'M8195A_4ch'))
        warndlg('PRBSes > 2^15 are only allowed with M8195A and large memory');
...
[s, fs, ~, numSamples] = calc_serial(handles, 'download', []);
set(handles.editNumSamples, 'String', sprintf('%d', numSamples));
if (~isempty(s))
...
[s, fs, dataRate, ~] = calc_serial(handles, 'clock', clockPat);
if (~isempty(s))
...
function [s, fs, dataRate, numSamples] = calc_serial(handles, fct, param)
s = [];
fs = 0;
dataRate = 0;
numSamples = 0;
...
switch (fct)
    case 'code' % generate MATLAB code
        ...
        if (isLargeData(handles))
            ...
            downloadStr = sprintf('iqdownload(s, fs, ''segmentNumber'', %g, ''channelMapping'', %s);\n', segmentNum, channelMapping);
        end
        s = sprintf(['[s, fs, nSym, nSamples] = iserial(''dataRate'', %g, ''sampleRate'', %g, ...\n' ...
            ...
            '    ''sscFreq'', %g, ''sscDepth'', %g, ''symbolShift'', %d,...\n' ...
            ...
            symbolShift, amplitude, dutyCycle / 100, correction, downloadStr);
        fs = 0;
        dataRate = 0;
    case {'download' 'display' 'save' 'clock'} % generate the actual waveform
        [s, fs, numBits, numSamples] = iserial('dataRate', dataRate, 'sampleRate', sampleRate, ...
            ...
            'sscFreq', sscFreq, 'sscDepth', sscDepth / 100, 'symbolShift', symbolShift, ...
...
%strcat(handles.DirPath, get(handles.editFilename,'string'))
data = ptrnfile2data(strcat(handles.DirPath, '\', get(handles.editFilename,'String')));
if (isempty(data))
