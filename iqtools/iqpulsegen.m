...
sampleRate = 8e9;
pw = 10;    % 'pw' - pulse width in samples (can be a vector)
rise = 20;
fall = 20;
off = 50;
...
if (correction)
    pulse = real(iqcorrection(real(pulse), sampleRate));
end
...
if (k * length(pulse) < arbConfig.minimumSegmentSize)
    k = k * ceil(arbConfig.minimumSegmentSize / (k * length(pulse)));
end
